/**
 * @fileOverview kissy-pagination 组件的安全适配器
...
KISSY.add(function (S, Gallery) {
    ...
    function init(frameGroup) {
         ...
         */
        function SafePagination(el, config) {
            this.inner = new Pagination(el, config);//内部实际封装了真正的组件构造函数
        }

        //为我们门面模式包装的构造函数添加需要开放给外部使用的原型方法，on和destory只是举例，哪些需要暴露的方法就写哪些就行了
        SafePagination.prototype.on = function (type,fnc) {
            //this.inner.on(type,fnc);
        };

        //为我们门面模式包装的构造函数添加需要开放给外部使用的原型方法
        SafePagination.prototype.destory = function () {
            //this.inner.destory();
        };
        ...
        frameGroup.markCtor(SafePagination);

        frameGroup.grantMethod(SafePagination, "on");
        frameGroup.grantMethod(SafePagination, "destory");
        ...
        return function (context) {
            ...
            //最终需要返回给
            return {
                //这里的 kissy-pagination 会自动放到KISSY.Pagination:
            Pagination: frameGroup.markFunction(function () {
                    return new SafePagination(arguments[0], cajaAFTB.untame(arguments[1]));
                }),
                kissy:true
            }
        }
